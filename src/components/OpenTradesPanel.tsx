    ...
    <div className="flex-1 flex flex-col bg-[#121418] h-full overflow-hidden">
      ...
      <div className="flex px-2 sm:px-4 border-b border-white/5 bg-black/20 shrink-0 overflow-x-auto no-scrollbar">
        {[
            ...
            { id: 'CLOSED', label: 'History', icon: History, count: trades.filter(t => t.status === 'CLOSED' && t.accountType === accountType).length }
        ].map(tab => (
            <button 
                key={tab.id}
                ...
                <table className="w-full text-left text-[11px] font-mono border-separate border-spacing-0 min-w-[600px]">
                    <thead className="bg-black/60 text-slate-600 uppercase tracking-widest text-[9px] sticky top-0 z-20">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-right font-black">Current/Close</th>
                    ...
                    <tbody className="divide-y divide-white/[0.02]">
                        {filtered.map(t => {
                            const price = t.status === 'CLOSED' ? (t.closePrice || t.entryPrice) : (prices[t.symbol] || t.entryPrice);
                            const isEditing = editingId === t.id;
                            
                            // Calculate live P/L for open trades based on current price
                            let displayPnl = t.pnl || 0;
                            if (t.status === 'OPEN') {
                              const diff = t.type.includes('BUY') ? price - t.entryPrice : t.entryPrice - price;
                              displayPnl = diff * (ASSETS[t.symbol]?.CONTRACT_SIZE || 1) * t.lotSize;
                            }
                            ...
                            return (
                                <tr key={t.id} className="hover:bg-white/[0.01] group transition-colors">
                                    ...
                                    <td className={`px-4 py-2 text-right font-bold text-xs ${displayPnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                        {displayPnl >= 0 ? '+' : ''}{displayPnl.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </td>
                                    <td className="px-4 py-2 text-right">
