...
const Navigation: React.FC<NavigationProps> = ({ currentView, onNavigate, onOpenLogin, userEmail, onLogout }) => {
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <div className="flex items-center gap-2">
              {userEmail ? (
                ...
                <button 
                  onClick={() => onOpenLogin ? onOpenLogin() : handleNavigate('TERMINAL')}
                  className="hidden sm:block px-4 py-2 rounded-full text-[9px] sm:text-[10px] font-bold text-zinc-400 hover:text-white hover:bg-white/10 transition-all uppercase tracking-widest border border-white/5 active:scale-95 cursor-pointer"
                >
                  Sign In
                </button>
              )}
          ...
          <div className="absolute right-0 top-0 h-full w-[280px] bg-[#0b0c10] border-l border-white/10 p-6 pt-24 flex flex-col gap-4 shadow-2xl">
            ...
            <div className="mt-auto pt-6 border-t border-white/5 flex flex-col gap-3">
              ...
              {userEmail ? (
                ...
                <button 
                  onClick={() => { if (onOpenLogin) onOpenLogin(); else handleNavigate('TERMINAL'); setIsMobileMenuOpen(false); }}
                  className="w-full px-4 py-4 rounded-xl text-xs font-bold text-blue-500 hover:bg-blue-500/10 transition-all uppercase tracking-widest text-left cursor-pointer"
                >
                  Sign In
                </button>
              )}
